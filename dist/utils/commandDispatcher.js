...
const challengeService = __importStar(require("../services/challengeService")); // Now imports all your logic
/**
 ...
 */
async function dispatchCommand(payload, hostname) {
    ...
    const usernameToStore = user.displayname || user.username || platformId;
    try {
        ...
        logger_1.default.info(`Dispatching API command`, {
            userId: dbUser.id,
            platformId: platformId,
        ...
        if (command.startsWith('!push')) {
            ...
            if (firstArg === 'confirm') {
                ...
                const result = await challengeService.processPushConfirm(dbUser.id, platformId, platformName, quoteId);
                return {
            ...
            const result = await challengeService.processPushQuote(dbUser.id, platformId, platformName, challengeId, quantity);
            return {
        ...
        switch (command) {
            case '!challengesubmit':
                ...
                const linkResult = await challengeService.processSubmissionLinkGeneration(dbUser.id, platformId, platformName, usernameToStore, durationArg, hostname // Pass hostname for URL construction
                );
                logger_1.default.info('Challenge submission link generated.', linkResult.details);
                return {
            ...
            case '!digout':
                ...
                const digoutResult = await challengeService.processDigout(dbUser.id, platformId, platformName, digoutChallengeId);
                return {
            ...
            case '!remove':
                ...
                const removeOption = (parsedArgs[1]?.toLowerCase() || 'community_forfeit');
            ...
            case '!disrupt':
                // COMMAND: !disrupt
                const disruptMessage = await challengeService.processDisrupt(dbUser.id, platformId, platformName);
        ...
        logger_1.default.error(`Critical Dispatch Error for command: ${fullCommand}`, {
