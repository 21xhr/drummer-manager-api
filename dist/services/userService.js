...
exports.processUserStats = processUserStats;
 ...
 */
async function isGameMaster(platformName, platformId) {
 ...
 */
async function createNewCentralUser() {
    const user = await prisma_1.default.user.create({
        data: {
            // These are the only required fields for a brand new user
            dailyChallengeResetAt: (0, challengeService_1.getNextDailyResetTime)(),
 ...
 */
async function findOrCreateUser(platformUser) {
...
async function processUserStats(userId) {
    ...
    user.accounts.forEach(account => {
        ...
        accountDetails.push(`  - **${account.platformName}** (@${display}): **${account.currentBalance.toLocaleString()}** NUMBERS`);
    });
    // --- 2. Format and Construct Message ---
    // Helper function for formatting numbers
    const format = (n) => (typeof n === 'bigint' ? n.toString() : n).toLocaleString();
    ...
    const statsMessage = `📊 **Stats for User ID: ${user.id}** (${user.accounts.length} Accounts Linked)\n` +
        `\n` +
        `💰 **[ECONOMIC BREAKDOWN]**\n` +
