...
exports.processDailyUserTick = processDailyUserTick;
exports.checkOneOffContiguity = checkOneOffContiguity;
exports.enforceRecurringChallengeCadence = enforceRecurringChallengeCadence;
exports.runDailyMaintenance = runDailyMaintenance;
 ...
 */
async function processDailyUserTick(currentStreamDay) {
    // 1. Fetch all users who haven't been processed for the current Stream Day
    const usersToUpdate = await prisma_1.default.user.findMany({
        where: {
    ...
    try {
        const executingCount = await prisma_1.default.challenge.count({
 ...
 */
async function checkOneOffContiguity() {
    // The presence in 'InProgress' for a ONE_OFF at the maintenance time is the failure condition.
    const updateResult = await prisma_1.default.challenge.updateMany({
        where: {
            status: client_1.ChallengeStatus.IN_PROGRESS,
            durationType: client_1.DurationType.ONE_OFF,
            isExecuting: false, // Prevents premature archival of a currently running challenge.
        },
        data: {
            status: client_1.ChallengeStatus.FAILED,
 ...
 */
async function enforceRecurringChallengeCadence() {
    const now = new Date();
    let challengesChecked = 0;
    // 1. Find all IN_PROGRESS Recurring Challenges that are NOT currently executing
    const recurringChallenges = await prisma_1.default.challenge.findMany({
        where: {
            status: client_1.ChallengeStatus.IN_PROGRESS,
            durationType: client_1.DurationType.RECURRING,
            isExecuting: false,
        }
    });
    ...
    for (const challenge of recurringChallenges) {
        ...
        if (challenge.cadenceUnit === client_1.CadenceUnit.DAILY) {
            periodDays = 1;
        }
        else if (challenge.cadenceUnit === client_1.CadenceUnit.MONTHLY) {
            periodDays = 30;
        }
        else if (challenge.cadenceUnit === client_1.CadenceUnit.CUSTOM_DAYS) {
        ...
        if (periodPassed) {
            ...
            if (challenge.cadenceProgressCounter < requiredCount) {
                ...
                await prisma_1.default.challenge.update({
...
async function runDailyMaintenance() {
    ...
    const updatedStat = await prisma_1.default.streamStat.update({
