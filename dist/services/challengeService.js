...
exports.getNextDailyResetTime = getNextDailyResetTime;
exports.processSubmissionLinkGeneration = processSubmissionLinkGeneration;
exports.processAutomaticSessionTick = processAutomaticSessionTick;
exports.getCurrentDailySubmissionContext = getCurrentDailySubmissionContext;
exports.processPushQuote = processPushQuote;
exports.processPushConfirm = processPushConfirm;
exports.processDigout = processDigout;
exports.processChallengeSubmission = processChallengeSubmission;
exports.archiveExpiredChallenges = archiveExpiredChallenges;
exports.finalizeInProgressChallenge = finalizeInProgressChallenge;
exports.processExecuteChallenge = processExecuteChallenge;
exports.getActiveChallenges = getActiveChallenges;
exports.processRemove = processRemove;
exports.processDisrupt = processDisrupt;
exports.setChallengeStatusByAdmin = setChallengeStatusByAdmin;
const prisma_1 = __importDefault(require("../prisma"));
const client_1 = require("@prisma/client");
const streamService_1 = require("./streamService");
const jwtService_1 = require("./jwtService");
const lumiaService_1 = require("./lumiaService");
 ...
 */
function parseCadenceRequiredCount(sessionCadenceText) {
    if (!sessionCadenceText)
        return 1;
 ...
 */
async function processSubmissionLinkGeneration(userId, platformId, platformName, username, duration, reqHostname) {
    ...
    const tokenDuration = (0, jwtService_1.validateDuration)(duration); // Use validateDuration from jwtService
    // NOTE: The TokenPayload interface in jwtService requires userId, platformId, platformName, username
    const token = (0, jwtService_1.generateToken)({ userId, platformId, platformName, username }, tokenDuration);
    // 3. Dynamic URL Construction
    const isLocalHost = reqHostname === 'localhost' || reqHostname === '127.0.0.1' || reqHostname === '0.0.0.0';
    const WEBFORM_BASE_URL = isLocalHost
    ...
    const chatResponse = `Please use the following secure link to submit your challenge (valid for ${tokenDuration}).\n` +
 ...
 */
async function processAutomaticSessionTick() {
    const now = new Date();
    const currentTimeMs = now.getTime();
    // TEMPORARY DEBUG LOG
    logger_1.default.info('Auto Tick: Checking for executing challenge...');
    // 1. Find the currently executing challenge
    const executingChallenge = await prisma_1.default.challenge.findFirst({
    ...
    if (!executingChallenge.timestampLastSessionTick) {
        ...
        logger_1.default.warn(`Auto Tick: Skipping Challenge #${executingChallenge.challengeId}. Clock not initialized/is NULL.`);
        return null;
    }
    ...
    logger_1.default.info(`Auto Tick: Session expired for Challenge #${executingChallenge.challengeId}. Processing session increment.`);
    return prisma_1.default.$transaction(async (tx) => {
        ...
        if (isCompleted) {
            updateData.status = client_1.ChallengeStatus.COMPLETED;
            updateData.timestampCompleted = txNow.toISOString();
            logger_1.default.info(`Auto Tick: Challenge #${executingChallenge.challengeId} is COMPLETED.`);
        }
 ...
 */
async function getCurrentDailySubmissionContext(userId) {
 ...
 */
function calculateSubmissionCost(challengeCountToday) {
    const N = challengeCountToday;
    let cost = SUBMISSION_BASE_COST * ((N + 1) * (N + 1));
    // Apply LIVE STREAM Discount (21% off)
    if ((0, streamService_1.isStreamLive)()) {
        cost = Math.ceil(cost * LIVE_DISCOUNT_MULTIPLIER); // Round up after discount
    }
    return cost;
}
/**
 * Applies the live discount (21% off) to a calculated cost,
 * using BigInt arithmetic to ensure precision, and rounding up.
 * @param cost The calculated cost as a BigInt.
 * @returns The discounted cost as a BigInt.
 */
function applyLiveDiscount(cost) {
    if ((0, streamService_1.isStreamLive)()) {
        // 1. Calculate the discounted value (cost * 0.79)
        const discountedCost = cost * LIVE_DISCOUNT_MULTIPLIER_NUMERATOR;
        // 2. Perform ceiling division for final rounding: ceil(A / B) = (A + B - 1) / B
        // We use ceil() to round up, ensuring the charge is never lower than the calculation.
        return (discountedCost + DISCOUNT_DIVISOR - 1n) / DISCOUNT_DIVISOR;
    }
    return cost;
}
////////////////////////////////////////////////////////////////////////////////////////
// PROCESS PUSH QUOTE
////////////////////////////////////////////////////////////////////////////////////////
async function processPushQuote(userId, platformId, platformName, challengeId, quantity) {
    // Implement a Quantity Cap
    const MAX_QUOTE_QUANTITY = 210;
    if (quantity > MAX_QUOTE_QUANTITY) {
        throw new Error(`Maximum push quantity is ${MAX_QUOTE_QUANTITY}. Please try a smaller amount.`);
    }
    // 1. Fetch Challenge Details for validation and base cost
    const challenge = await prisma_1.default.challenge.findUnique({
        where: { challengeId: challengeId },
    });
    if (!challenge || challenge.status !== client_1.ChallengeStatus.ACTIVE) {
    ...
    const userPushRecord = await prisma_1.default.push.aggregate({
        _sum: { quantity: true },
        where: { userId: userId, challengeId: challengeId },
    });
    const currentUserPushCount = userPushRecord._sum.quantity ?? 0;
    let quotedCostBigInt = BigInt(0); // Initialize as BigInt
    ...
    const quoteId = v4();
    await prisma_1.default.tempQuote.create({
        data: {
...
////////////////////////////////////////////////////////////////////////////////////////
async function processPushConfirm(userId, platformId, platformName, quoteId) {
    const currentStreamSessionId = (0, streamService_1.getCurrentStreamSessionId)();
    ...
    return prisma_1.default.$transaction(async (tx) => {
        ...
        let newAuthoritativeBalance;
        try {
            // Use the platformId passed in the function signature
            const lumiaResult = await (0, lumiaService_1.deductNumbersViaLumia)(platformId, pushTransactionCost);
            ...
            logger_1.default.error(`Lumia Push Deduction Failed for User ${userId} (Cost: ${pushTransactionCost}): ${error instanceof Error ? error.message : 'Unknown error'}`, { userId, platformId: platformId });
            await tx.tempQuote.delete({ where: { quoteId: quote.quoteId } });
        ...
        // --- 5. CLEANUP ---
        await tx.tempQuote.delete({ where: { quoteId: quote.quoteId } });
        return { updatedChallenge, transactionCost: pushTransactionCost, quantity: quote.quantity, updatedAccount: updatedAccount };
    });
}
////////////////////////////////////////////////////////////////////////////////////////
// PROCESS DIGOUT
 ...
 */
async function processDigout(userId, platformId, platformName, challengeId) {
    const currentStreamSessionId = (0, streamService_1.getCurrentStreamSessionId)();
    const transactionTimestamp = new Date().toISOString();
    const result = await prisma_1.default.$transaction(async (tx) => {
        ...
        if (challenge.status !== client_1.ChallengeStatus.ARCHIVED) {
            // Clarified message: Digout is only for time-expired challenges.
            const eligibleStatus = client_1.ChallengeStatus.ARCHIVED;
        ...
        const totalSpentBigInt = challenge.totalNumbersSpent; // Total spent is now BigInt
        // 2a. Calculate numerator: totalSpent * 21
        const digoutNumerator = totalSpentBigInt * DIGOUT_PERCENTAGE_NUMERATOR;
        // 2b. Apply BigInt ceiling division: ceil(A / B) = (A + B - 1) / B
        const digoutCostBigInt = (digoutNumerator + DISCOUNT_DIVISOR - 1n) / DISCOUNT_DIVISOR;
        // 2c. Convert the precise BigInt result back to a number (required for Lumia and `Int` fields)
        const digoutTransactionCost = Number(digoutCostBigInt);
        // -------------------------------------------------------------
        // Check Account balance
        ...
        try {
            // Use the platformId passed in the function signature
            const lumiaResult = await (0, lumiaService_1.deductNumbersViaLumia)(platformId, digoutTransactionCost);
            newAuthoritativeBalance = lumiaResult.newBalance;
        }
        catch (error) {
            logger_1.default.error(`Lumia Digout Deduction Failed for User ${userId} (Cost: ${digoutTransactionCost}): ${error instanceof Error ? error.message : 'Unknown error'}`, { userId, platformId: platformId });
            const errorMessage = error instanceof Error ? error.message : 'Unknown payment failure.';
            throw new Error(`Payment failed. ${errorMessage.includes('Insufficient funds') ? 'Insufficient funds.' : 'Lumia connection error.'}`);
        }
    ...
    (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_DIGGED_OUT, result.updatedChallenge);
...
////////////////////////////////////////////////////////////////////////////////////////
async function processChallengeSubmission(userId, platformId, platformName, challengeText, totalSessions, durationType, sessionCadenceText, cadenceUnit) {
    const currentStreamSessionId = (0, streamService_1.getCurrentStreamSessionId)();
    const transactionTimestamp = new Date().toISOString();
    // VALIDATION: Ensure required values are provided and valid
    if (!durationType || !Object.values(client_1.DurationType).includes(durationType)) {
    ...
    if (durationType === client_1.DurationType.RECURRING && !sessionCadenceText) {
    ...
    return prisma_1.default.$transaction(async (tx) => {
        ...
        const { dailySubmissionCount: N, baseCostPerSession: submissionCost } = await getCurrentDailySubmissionContext(userId);
        ...
        try {
            ...
            const lumiaResult = await (0, lumiaService_1.deductNumbersViaLumia)(platformId, submissionCost);
            ...
            logger_1.default.error(`Lumia Deduction Failed for User ${userId} (Cost: ${submissionCost}): ${error instanceof Error ? error.message : 'Unknown error'}`, { userId, platformId: accountContext.platformId });
            const errorMessage = error instanceof Error ? error.message : 'Unknown payment failure.';
            // Only expose a user-friendly message
            throw new Error(`Payment failed. ${errorMessage.includes('Insufficient funds') ? 'Insufficient funds.' : 'Lumia connection error.'}`);
        }
        // Parse required count for recurring challenges
        let cadenceRequiredCount;
        if (durationType === client_1.DurationType.RECURRING) {
        ...
        const newChallenge = await tx.challenge.create({
            data: {
                ...
                status: client_1.ChallengeStatus.ACTIVE, // Required field (Always starts Active)
        ...
        (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_SUBMITTED, newChallenge);
        // 10. Return results
        return {
            newChallenge,
            cost: submissionCost,
 ...
 */
async function archiveExpiredChallenges() {
    ...
    const result = await prisma_1.default.$transaction(async (tx) => {
        ...
        const updateResult = await tx.challenge.updateMany({
            ...
            data: {
                status: client_1.ChallengeStatus.ARCHIVED,
            },
        });
 ...
 */
async function finalizeInProgressChallenge() {
    // ... (find executingChallenge)
    const executingChallenge = await prisma_1.default.challenge.findFirst({
        where: { status: client_1.ChallengeStatus.IN_PROGRESS, isExecuting: true },
    });
    if (!executingChallenge) {
    ...
    let completedChallenge = await prisma_1.default.challenge.update({
        where: { challengeId: executingChallenge.challengeId },
        data: updateData
    });
    const statusText = isCompleted ? client_1.ChallengeStatus.COMPLETED : `finished session ${nextSessionCount} of ${completedChallenge.totalSessions}. Status remains InProgress.`;
...
////////////////////////////////////////////////////////////////////////////////////////
async function processExecuteChallenge(challengeId) {
    return prisma_1.default.$transaction(async (tx) => {
        const transactionTimestamp = new Date().toISOString();
        ...
        if (challenge.status !== client_1.ChallengeStatus.ACTIVE && challenge.status !== client_1.ChallengeStatus.IN_PROGRESS) {
            throw new Error(`Challenge #${challengeId} cannot be executed. Status must be 'ACTIVE' or 'IN_PROGRESS'`);
        }
        // 3. Execute the new challenge
        const newStatus = challenge.status === client_1.ChallengeStatus.ACTIVE ? client_1.ChallengeStatus.IN_PROGRESS : challenge.status;
        const updateData = {
            status: newStatus,
            isExecuting: true,
        ...
        if (challenge.status === client_1.ChallengeStatus.ACTIVE && challenge.durationType === 'RECURRING') {
        ...
        if (executingChallenge.isExecuting) {
            ...
            (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_EXECUTED, executingChallenge);
        }
...
async function getActiveChallenges() {
    return prisma_1.default.challenge.findMany({
        where: { status: client_1.ChallengeStatus.ACTIVE },
        orderBy: { totalNumbersSpent: 'desc' },
    });
}
async function processRemove(authorUserId, challengeId, option = 'community_forfeit' // Default to Community Forfeit
) {
    ...
    const result = await prisma_1.default.$transaction(async (tx) => {
        ...
        const unremovableStatuses = [
            client_1.ChallengeStatus.ARCHIVED,
            client_1.ChallengeStatus.AUCTIONED,
            client_1.ChallengeStatus.IN_PROGRESS,
            client_1.ChallengeStatus.COMPLETED,
            client_1.ChallengeStatus.REMOVED
        ];
            ...
            await tx.account.update({
                where: {
                    platformId_platformName: {
                        ...
                        platformName: client_1.PlatformName.GAME_MASTER // Designated sink platform
                    }
                },
                data: {
        ...
        const updatedChallenge = await tx.challenge.update({
            where: { challengeId: challengeId },
            data: {
                status: client_1.ChallengeStatus.REMOVED,
                isExecuting: false
            },
        });
        ...
        return {
            ...
            totalRefundsAmount,
            option: option, // Cast to the expected type
            fundsSinkText,
            toAuthor,
            toExternalPushers,
            toCommunityChest
    ...
    if (requiresExternalRefund && attemptedRefundCount > 0) {
        ...
        const accountsToRefund = await prisma_1.default.account.findMany({
        ...
        const refundPromises = result.allExternalRefundsToProcess.map(async (refund) => {
            ...
            if (!platformId) {
                logger_1.default.error(`Refund Failed: User ${refund.userId} is missing platformId.`);
                return; // Failed to refund, no platform ID
            }
            try {
                // ⭐ CRITICAL: Call the authoritative credit API
                await (0, lumiaService_1.addNumbersViaLumia)(platformId, refund.refundAmount);
                ...
                logger_1.default.error(`Lumia Refund Failed for User ${refund.userId} (Amount: ${refund.refundAmount}): ${errorMessage}`, { userId: refund.userId, platformId });
            }
        });
    ...
    (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_REMOVED_BY_AUTHOR, result.updatedChallenge);
    return {
 ...
 */
async function processDisrupt(userId, platformId, platformName) {
    const currentStreamSessionId = (0, streamService_1.getCurrentStreamSessionId)();
    const transactionTimestamp = new Date().toISOString();
    return prisma_1.default.$transaction(async (tx) => {
        ...
        try {
            // Use the platformId passed in the function signature
            const lumiaResult = await (0, lumiaService_1.deductNumbersViaLumia)(platformId, DISRUPT_COST);
            newAuthoritativeBalance = lumiaResult.newBalance;
        }
        catch (error) {
            logger_1.default.error(`Lumia Disrupt Deduction Failed for User ${userId} (Cost: ${DISRUPT_COST}): ${error instanceof Error ? error.message : 'Unknown error'}`, { userId, platformId: platformId });
            const errorMessage = error instanceof Error ? error.message : 'Unknown payment failure.';
            throw new Error(`Payment failed. ${errorMessage.includes('Insufficient funds') ? 'Insufficient funds.' : 'Lumia connection error.'}`);
        }
        ...
        return ("Congratulations, you just paid 2100 NUMBERS to commit pre-release performance art. " +
 ...
 */
async function setChallengeStatusByAdmin(challengeId, newStatus) {
    // 1. Basic Validation (ensure the status is valid)
    if (!Object.values(client_1.ChallengeStatus).includes(newStatus)) {
    ...
    const updateData = {
        ...
        ...(newStatus !== client_1.ChallengeStatus.IN_PROGRESS && {
            isExecuting: false
        }),
        // Optionally, record the completion timestamp if setting to a final state
        ...(newStatus === client_1.ChallengeStatus.COMPLETED && {
            timestampCompleted: new Date().toISOString()
        })
    };
    try {
        const updatedChallenge = await prisma_1.default.challenge.update({
            where: { challengeId: challengeId },
            data: updateData
        });
        ...
        if (newStatus === client_1.ChallengeStatus.COMPLETED) {
            // GM force-completed the challenge
            (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_COMPLETED, updatedChallenge);
        }
        else if (newStatus === client_1.ChallengeStatus.REMOVED) {
            // GM set the final status to REMOVED
            (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_REMOVED, updatedChallenge);
        }
        else if (newStatus === client_1.ChallengeStatus.ACTIVE) {
            // GM is manually activating/reviving a challenge
            (0, eventService_1.publishChallengeEvent)(eventService_1.ChallengeEvents.CHALLENGE_ACTIVATED_BY_GM, updatedChallenge);
        }
